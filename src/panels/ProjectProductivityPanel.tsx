...
import { CollapsibleSection } from '../components/productivity/CollapsibleSection'
...
export default function ProjectProductivityPanel({ projectId, initialContractId }: ProjectProductivityPanelProps) {
  ...
  const availableContracts = useMemo(() => getAvailableContracts(items), [items])
  const availableWorkStreams = useMemo(() => getAvailableWorkStreams(items), [items])
  ...
  const grouped = useMemo(() => groupByComponent(filteredItems), [filteredItems])
  ...
  const constructionItems = grouped.filter((item) => item.workStream === 'Construction')
  ...
  const actualPercent = totals.plannedQty === 0 ? 0 : (totals.actualQty / totals.plannedQty) * 100
  const quantityVariance = actualPercent - plannedPercent
  const valuePercent = totals.plannedValue === 0 ? 0 : (totals.actualValue / totals.plannedValue) * 100
  ...
  return (
    <aside className="project-productivity pp-rightPanel" aria-label="Project productivity insights">
      {loading ? (
        ...
        <>
          <div className="productivity-heading">
            ...
            <span>
              Scope synced with {filters.contractId === 'ALL' ? 'all contracts' : filters.contractId} ·{' '}
          ...
          <CollapsibleSection title="Design Work Output">
            <ComponentProgressList items={designItems} />
          </CollapsibleSection>

          <CollapsibleSection title="Preparatory Work Output">
            <ComponentProgressList items={prepItems} />
          </CollapsibleSection>

          <CollapsibleSection title="Construction Work Output">
            <ComponentProgressList items={constructionItems} />
          </CollapsibleSection>

          <CollapsibleSection title="Project Quality Performance">
            <QualitySection data={quality} />
          </CollapsibleSection>

          <CollapsibleSection title="Schedule Performance Index (SPI)">
            <SpiSection data={spi} />
          </CollapsibleSection>
        </>
      )}
